...
IDE（integrated development environment），中文名为集成开发环境，顾名思义，是一种辅助程序开发人员开发软件的编辑器软件。它一般可以辅助程序员编写源代码，并编译打包源程序为可执行程序。

最原始的程序书写方式是程序员在类似于记事本一样的空白文件上书写所有的源代码，然后通过终端程序（terminal）输入终端命令，编译和执行我们所写的源代码。这就导致大量的重复工作和在编辑时由于疏忽大意导致的许多低级错误频出，于是程序员们思考如何打造一款方便大家高效编辑代码的软件，各种针对不同编程语言的程序开发软件（也就是IDE）应运而生。

针对C语言开发的主流IDE有：

* Microsoft Visual C++ 6.0（简称VC6.0，由微软公司发布于1998年，这款软件过于古老，原版只支持Windows XP系统，所以现在所使用的都是这个软件的兼容版，兼容了后来的Win7/Win8/Win10系统等）
* Microsoft Visual C++ 2010（简称VC2010，由微软公司发布于2010年4月，是目前全国计算机二级考试C语言的指定软件，但相比于更新的版本，它的功能依然略显简陋）
* Microsoft Visual Studio （简称VS，由微软公司发布并持续更新的可以支持整个软件生命周期开发和管理的开发工具集，它仅支持C/C++开发，还支持如Java、Python等，且支持Windows、Linux、Mac OS等操作系统）
* Dev C++ 

此外，还有一些比较轻量级的代码编辑软件，如Microsoft Visual Studio Code、Eclipse和Sublime等，这些工具主要的辅助功能是程序的源代码编辑，其余的项目部署和管理工作由程序员通过其他软件或手段自行实现。

Mac OS环境下，C/C++比较好的编辑器有Xcode和Visual Studio Code。

# 为什么我们使用的是C++软件？

C++是作为C语言的增强版出现的，它几乎包含了C所有的功能，并新增了关于面向对象为核心的类库。我们通过支持C和C++的开发软件来练习C程序完全没有问题。
