...
* a = a + 1.7; 等式右侧的运算结果是11.7，double类型，但是赋值给int变量，所以a = 11。
...
1. 运算符结合方向：
   * 只有三个从右向左：前缀++ 前缀-- 按位操作符(一元运算符)；三元操作符；赋值符
   * 其余全部从左向右

2. 所有双目运算符中，只有赋值运算符是右结合。

3. 优先级大概顺序：算数运算符 > 关系运算符 > 逻辑运算符(!比较高) > 赋值运算符 > 逗号运算符
4. 不需要刻意记忆，常用即可。
...
![image-20210126201226964](C_Programming_Sgg.assets/image-20210126201226964.png)

### 3 标识符命名规范

标识符：即变量、函数在命名时所用的字符序列。
...
2. 数字不能开头
3. 不能使用关键字和保留字，但可以包含这些字
4. C语言严格区分大小写
5. 标识符不能包含空格、- 等符号

命名规范：

1. 程序中不能出现仅靠大小写来区分的相似的标识符——不利于程序可读性
2. 所有宏定义、枚举常数、常量都用大写字母命名，用下划线分隔单词
3. 定义变量时就要初始化。因为编译器不一定会为只声明未赋值的变量清空其内存，异常值进入程序可能导致程序异常退出。
4. 变量名或方法名命名时优先使用小写，可以用下划线分割单词，也可以使用小驼峰式组合单词。

![image-20210126202417154](C_Programming_Sgg.assets/image-20210126202417154.png)
